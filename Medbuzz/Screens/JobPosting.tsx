...
const JobPosting = () => {
  ...
  //console.log('this is the description: ', job.public_job_desc);
  //console.log(job);
  ...
  const onConfirmPress = async () => {
    ...
    if (sendResumeSelected) {
      await RNFetchBlob.fs
        .exists(resumePath)
        .then(resumeExists => {
          if (resumeExists) console.log('File exists.\nResume sent.');
          else {
    ...
    if (sendLicensesSelected) {
      await RNFetchBlob.fs
        .exists(licensePath)
        .then(licenseExists => {
          if (licenseExists) console.log('File exists.\nLicense sent.');
          else {
            console.log('File does not exist.');
            problems = true;
          }
    ...
    if (sendDegreeSelected) {
      await RNFetchBlob.fs
        .exists(degreePath)
        .then(degreeExists => {
          if (degreeExists) console.log('File exists.\nDegree sent.');
          else {
            console.log('File does not exist.');
            problems = true;
          }
    ...
    if (sendReferencesSelected) {
      await RNFetchBlob.fs
        ...
        .then(referenceExists => {
          if (referenceExists) console.log('File exists.\nReference sent.');
          else {
            console.log('File does not exist.');
            problems = true;
          }
    ...
    if (sendVaccinationSelected) {
      await RNFetchBlob.fs
        ...
        .then(vaccinationExists => {
          if (vaccinationExists) console.log('File exists.\nVaccination sent.');
          else {
            console.log('File does not exist.');
            problems = true;
          }
